...
Original file is located at
    https://colab.research.google.com/drive/17QMq3QV-n9sSlHPFsnXarE-H1L_Ho7VA
...
cup = pd.read_csv('/content/drive/MyDrive/DATA/WorldCupMatches.csv')

"""**Q2** Using seaborn's displot draw density curves to show the distributions of Home Team Goals for every year. """

sns.displot(cup, x="Home Team Goals", kind="kde", hue = "Year")
...
labels=[y if y%10==0 else None for y in list(cup.Year.unique())]

ax = sns.violinplot(data=cup, x='Year', y='Home Team Goals')
...
jp.joyplot(data=cup, column=['Home Team Goals', 'Away Team Goals'], by='Year', legend = True)
...
goals = cup.groupby(['Home Team Initials'])['Home Team Goals'].count().reset_index()

topFive = goals.sort_values(["Home Team Initials"], ascending=True)

ax = topFive[0:5].plot.bar(x='Home Team Initials', y='Home Team Goals', rot=0)
...
jp.joyplot(data=cup[25:30], column=['Home Team Goals', 'Away Team Goals'], by='Home Team Initials', ylim = 'own', legend = True)
...
pplot(data=cup, x="Home Team Goals", y="Away Team Goals", kind='qq', display_kws={"identity":True}, height=4, aspect=2)